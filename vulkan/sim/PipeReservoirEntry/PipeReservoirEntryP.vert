...
void main(){
	...
	if(!PipeReservoirIsActive(uint(index)))
	{
		gl_Position = vec4(0.0, 0.0, 0.0, 1.0);
		gl_PointSize = 0.0;
		return;
		...
		|| max_x >= float(WIDTH) || max_y >= float(HEIGHT) || max_z >= float(DEPTH))
	{
			...
			debugPrintfEXT("Boundary->P:=%d,min:<%0.3f,%0.3f,%0.3f> max:<%0.3f,%0.3f,%0.3f>",
				index,min_x,min_y,min_z,max_x,max_y,max_z);
		#endif
		// Check the location of the particle to see if is in bounds or stores defalut negative values.
		#if 1 && defined(DEBUG)
			debugPrintfEXT("Boundary->P:=%d,act:<%0.3f,%0.3f,%0.3f>",
				index,cx,cy,cz);
		#endif
