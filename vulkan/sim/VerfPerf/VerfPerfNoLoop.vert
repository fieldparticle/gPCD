...
uint AddSlot(uint CornerIndex, uint ParticleID)
{
	...
	{
		MaxCellOccupancyError(cellOccupancySlot,ParticleID);
		return 1;
	}
...
void MaxCellOccupancyError(uint CellOccupancySlot,uint ParticleID)
{
	...
	// then report it.
	#if 1 && defined(DEBUG)
		debugPrintfEXT("ParticleVerfPerf slot>F:%u,P:%d,MAX_CELL_OCCUPANY:%d,at loc: %d loc<%0.2f,%0.2f,%0.2f>",
		uint(ShaderFlags.frameNum),ParticleID,MAX_CELL_OCCUPANY,CellOccupancySlot,P[ParticleID].PosLocA.x,P[ParticleID].PosLocA.y,P[ParticleID].PosLocA.z);
