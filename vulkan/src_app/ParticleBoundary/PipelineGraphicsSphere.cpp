...
void PipelineGraphicsSphere::Create(SwapChainObj* SCO, ResourceContainerObj* RCO, RenderPassObj* RPO)
{
    m_SCO = SCO;
    m_RPO = RPO;
    m_RCO = RCO;
    ...
    m_WireFlag = m_App->m_CFG->m_WireFlag;
    
    createPipeline();
    AddBoundaryPipe();
};
void PipelineGraphicsSphere::AddBoundaryPipe()
{

    ConfigObj* cfg = m_App->m_CFG;
    auto vertShaderCode = ReadFile(cfg->m_vertShaderBoundary);
    auto fragShaderCode = ReadFile(cfg->m_fragShaderBoundary);


    VkShaderModule vertShaderModule = createShaderModule(vertShaderCode, cfg->m_vertShaderBoundary);
    VkShaderModule fragShaderModule = createShaderModule(fragShaderCode, cfg->m_fragShaderBoundary);
...
void PipelineGraphicsSphere::createPipeline()
{
        ConfigObj* cfg = m_App->m_CFG;
        // Viewport State
	    //
