...
uint32_t ResourceLightingParticle::CalcSideLength(size_t PartPerCell)
{
	uint32_t ii = 0;
	while (1)
	{
...
float ResourceLightingParticle::CalcSpeedLimit(float max_vel, float radius)
{
	return radius / (3 * max_vel);
}
void ResourceLightingParticle::Create(uint32_t BindPoint)
{
...
#include <cstdint>
	...
	while (input_file.peek() != EOF)
	{
		...
		if (true)
		{
			if (part_pos.rx < 0.5 || part_pos.ry < 0.5 || part_pos.rz < 0.5)
			{
				...
				if (m_NumParticles != 0)
				{
					objtxt << m_Name << "ResourceLightingParticle::Particle location below bounds P:" <<
		...
		part.initial_vel_energy = glm::vec4(part_pos.vx, part_pos.vy, part_pos.vz, 1.0);
		part.photon_transport	= glm::vec4(0.0, 0.0, 0.0, 0.0);
	...
	size_t styrsz		= sizeof(LightingParticle);
	size_t spdata		= sizeof(pdata);
	

	uint32_t sizeParta = sizeof(LightingParticle);
	//m_BufSize = static_cast<uint64_t>(sizeof(LightingParticle)) * m_NumParticles;
	m_BufSize = static_cast<uint64_t>(sizeof(LightingParticle)) * m_NumParticles;
	...
	{
		std::ostringstream  objtxt;
		objtxt << m_Name << "ResourceLightingParticle::Particle count does not match: Number read from tst file:" 
	...
	if (m_BufSize != m_Particles.size() * sizeof(LightingParticle))
	{
		std::ostringstream  objtxt;
		size_t num_p = m_Particles.size();
		size_t psize = m_Particles.size() * sizeof(LightingParticle);
		size_t struct_size = sizeof(LightingParticle);
		objtxt << m_Name << "ResourceLightingParticle::Buffer size Mismatch: BufSize:"
			<< m_BufSize
	...
	{
		std::ostringstream  objtxt;
		objtxt << m_Name << "ResourceLightingParticle::Create Particle size:" 
	...
	{
		std::ostringstream  objtxt;
		objtxt << m_Name << "ResourceLightingParticle::Create Particle Zero Problem:m_NumParticles="
...
void ResourceLightingParticle::ClearTempMemory()
{
	m_Particles.clear();
	std::vector<LightingParticle> empty;
	m_Particles.swap(empty);
}

void ResourceLightingParticle::CreateLayout()
{
...
ResourceLightingParticle::GetBindingDescription()
{
	
	m_BindingDescription.binding = m_BindPoint;
	m_BindingDescription.stride = sizeof(LightingParticle);
...
ResourceLightingParticle::GetAttributeDescriptions()
{
	...
	ad.offset = offsetof(LightingParticle, PosLocA);
	m_AttributeDescriptions.push_back(ad);

	ad.binding = m_BindPoint;
	ad.location = 1;
	ad.format = VK_FORMAT_R32G32B32A32_SFLOAT;
	ad.offset = offsetof(LightingParticle, PosLocB);
	m_AttributeDescriptions.push_back(ad);

	ad.binding = m_BindPoint;
	ad.location = 2;
	ad.format = VK_FORMAT_R32G32B32A32_SFLOAT;
	ad.offset = offsetof(LightingParticle, VelRadA);
	m_AttributeDescriptions.push_back(ad);

	ad.binding = m_BindPoint;
	ad.location = 3;
	ad.format = VK_FORMAT_R32G32B32A32_SFLOAT;
	ad.offset = offsetof(LightingParticle, Data);
	m_AttributeDescriptions.push_back(ad);
