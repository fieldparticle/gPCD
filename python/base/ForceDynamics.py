...
class ForceContactDynamics:
    ...
    def IsPhotonParticle(self, ParticleID):
        """Base FPM has no photon behavior."""
        return False

    def ShouldSkipParticlePair(self, SourceID, TargetID):
        """Base FPM processes ordinary particle pairs."""
        return False
        ...
        if self.IsPhotonParticle(SourceID):
            ...
            _deposit_fraction, deposit_rgb, remaining_mass = (
                self.TransferPhotonStrengthToSurface(
                    ...
                    normal,
                    sphere_material_id,
                    payload_rgb=self.MaterialColorRGB(sphere_material_id),
                )
            )
