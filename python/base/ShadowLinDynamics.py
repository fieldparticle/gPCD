...
class ShadowLinDynamics(ShadowDynamics):
    ...
    EPSILON = ShadowDynamics.GEO_EPSILON
    CONTACT_PARTICLE = ShadowDynamics.GEO_CONTACT_PARTICLE
    CONTACT_ACTIVE_THIS_FRAME = ShadowDynamics.GEO_CONTACT_ACTIVE_THIS_FRAME
    PHASE_COMPRESSION = ShadowDynamics.GEO_PHASE_COMPRESSION
    PHASE_RETURNING = ShadowDynamics.GEO_PHASE_RETURNING
    ...
    def ContactDynamics(self, SourceID):
        ...
        """
        if self.ShaderFlags.frameNum == 10:
            print(f"ContactDynamics called for SourceID {SourceID}")
    ...
    def CalculateContactImpulses(self, SourceID):
        ...
        for slot_index in range(source.contactCount):
            ...
            # This is the core of the linear contact response model: 
            # the raw impulse is proportional to the contact stiffness 
            # and overlap area, and then modulated by the available 
            # momentum and internal momentum to determine the actual 
            # compression and release impulses.
            raw_impulse = (
    ...
    def Move(self, SourceID):
        ...
        if not self.isValidParticleID(SourceID):
            return self.SetError(self.constants.GEO_ERROR_INVALID_SOURCE_ID)
        if dt <= 0.0:
            return self.SetError(self.constants.GEO_ERROR_INVALID_DT)
        ...
        if output_position.x < 1.0 or output_position.y < 1.0:
            return self.SetError(self.constants.GEO_ERROR_PARTICLE_OUT_OF_BOUNDS)
        return True
    ...
    def SetError(self, error_code):
        """Set the inherited shared collision error state and return failure."""
        return self.GeoSetError(error_code)
