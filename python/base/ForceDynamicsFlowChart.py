"""Generate the current ForceDynamics function flowchart.

This chart shows only the flow inside ForceDynamics.py.  At this stage the
...
DOT_FILE = OUTPUT_DIR / "ForceDynamicsFlow.dot"

FORCE_DYNAMICS = REPO_DIR / "python" / "base" / "ForceDynamics.py"
SOURCE_DOCUMENTATION = DOC_DIR / "ForceDynamicsSource.tex"

NODE_SOURCES = {
    "ContactDynamics": FORCE_DYNAMICS,
    "BeginSourceContactParms": FORCE_DYNAMICS,
    "InitializeContactState": FORCE_DYNAMICS,
    "GetContactState": FORCE_DYNAMICS,
    "GetParticleGeometry": FORCE_DYNAMICS,
    "GetParticlePosition": FORCE_DYNAMICS,
    "AppendContactSlot": FORCE_DYNAMICS,
    "GetContactSlots": FORCE_DYNAMICS,
    "GetCollisionPhase": FORCE_DYNAMICS,
    "GetCollisionInternalMomentum": FORCE_DYNAMICS,
    "CalculateContactParms": FORCE_DYNAMICS,
    "GetStartFrameVelocity": FORCE_DYNAMICS,
    "GetReducedMass": FORCE_DYNAMICS,
    "CalculateContactWeights": FORCE_DYNAMICS,
    "CalculateContactImpulses": FORCE_DYNAMICS,
    "GetPairStiffness": FORCE_DYNAMICS,
    "ApplySourceVelocityDelta": FORCE_DYNAMICS,
    "GetParticleMass": FORCE_DYNAMICS,
    "UpdateContactLedgers": FORCE_DYNAMICS,
    "SetCollisionInternalMomentum": FORCE_DYNAMICS,
    "SetCollisionPhase": FORCE_DYNAMICS,
    "SyncInternalMomentum": FORCE_DYNAMICS,
    "GetInternalMomentum": FORCE_DYNAMICS,
    "SetInternalMomentum": FORCE_DYNAMICS,
}
...
def node_url(node_id: str) -> str | None:
    if node_id not in NODE_SOURCES:
        return None
    section_text = f"\\ForceFunctionSection{{{node_id}}}"
...
def dot_node(
    ...
    if url:
        attrs.extend(
            [
                f'URL="{url}"',
                f'tooltip="Open the {label} section in ForceDynamicsSource.tex"',
                'target="_blank"',
            ]
        )
