"""Three-panel interactive plots for VerAForceDynamics reports."""
...
def _vera_force_report_mode():
    """Return the report stream currently assigned to VerAForceDynamics."""
...
def _pair_contact_series(item, frames):
    ...
    report_mode = _vera_force_report_mode()
...
def show_force_menu(batch_cfg):
    """Show the three-panel VerAForceDynamics plot window."""
    try:
        import matplotlib.pyplot as plt
        from matplotlib.widgets import RadioButtons
    except ModuleNotFoundError as exc:
        raise ModuleNotFoundError(
            "matplotlib is required for VerAForcePlots. "
...
def run_vera_force_plots(argv=None):
    """Run the interactive VerAForceDynamics plots."""
    import argparse

    parser = argparse.ArgumentParser(description="Plot VerAForceDynamics diagnostics.")
    parser.add_argument("path", help="Path to a run or batch cfg file.")
    parser.add_argument(
        "--show",
        action="store_true",
        help="Show the interactive VerAForceDynamics plot window.",
    )
