...
def render_lighting_particle() -> str:
    return """#ifndef LIGHTING_PARTICLE_GLSL
...
def render_fpml() -> str:
    lighting_prefix = """// Generated by tools/ExportForceDynamicsSimpleGLSL.py.
...
void reflecting_wall_photon_splat_deposit(
    uint splatIndex,
    RectangleWallSegment wall,
    vec3 point,
    vec3 rgb);
    ...
    vec3 rgb)
{
    ...
    if (REFLECTING_WALL_LIGHT_MAP_ENABLED == 0u ||
        wall.wallFlag != REFLECTING_WALL_LIGHT_MAP_SURFACE_ID ||
        REFLECTING_WALL_PHOTON_SPLAT_COUNT == 0u)
    {
        return;
    }
    if (splatIndex >= REFLECTING_WALL_PHOTON_SPLAT_COUNT)
    {
        return;
    }
    ...
    vec3 hitPoint =
        ...
        wall.uAxis * uCoord +
        wall.vAxis * vCoord +
        wall.inwardNormal * 0.01;
    ReflectingWallPhotonSplats[splatIndex].pos_radius =
        vec4(hitPoint, REFLECTING_WALL_PHOTON_SPLAT_RADIUS);
    ReflectingWallPhotonSplats[splatIndex].color =
        vec4(
            clamp(rgb * REFLECTING_WALL_REFLECTION_GAIN, vec3(0.0), vec3(1.0)),
            REFLECTING_WALL_PHOTON_SPLAT_ALPHA);
#endif
}
...
def write_outputs(
    ...
    (output_dir / "lighting_particle.glsl").write_text(
        render_lighting_particle(),
        encoding="utf-8",
        newline="\n",
    )
    ...
    (output_dir / "FPML.comp").write_text(
        render_fpml(),
        encoding="utf-8",
        newline="\n",
    )
