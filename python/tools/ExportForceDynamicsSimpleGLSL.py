...
{
    if (BOUNDARY_SPACE_PROXY_COUNT == 0u
            || targetParticleID < BOUNDARY_SPACE_FIRST_PARTICLE_ID) {
        return;
    }
    uint boundaryIndex = targetParticleID - BOUNDARY_SPACE_FIRST_PARTICLE_ID;
    if (boundaryIndex >= BOUNDARY_SPACE_PROXY_COUNT) {
        return;
    }
    BoundaryLight[boundaryIndex].rgb_valid =
