...
private:
    ...
    std::string physDevName = "NVIDIA RTX 3500 Ada Generation Laptop GPU";
    ...
    void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
        ...
        vkCmdResetQueryPool(commandBuffer, m_PerfQueryPool,
            0, m_QueryCount);
    ...
    double m_ExecutionTime = 0.0f;
    uint32_t m_TimeStampDivisor = 0;

    void SetTimeStamp(uint32_t currentBuffer)
    {
        vkCmdResetQueryPool(commandBuffer, m_PerfQueryPool, currentBuffer, currentBuffer * 2);
        vkCmdWriteTimestamp(commandBuffer,
    ...
    void CreateQueryPool()
    {
        ...
        m_QueryCount =  2; // REVIEW
        createInfo.queryCount = m_QueryCount;
        mTimeQueryResults.resize(m_QueryCount);

        VkResult result = vkCreateQueryPool(device, &createInfo, nullptr, &m_PerfQueryPool);
    ...
    void FetchRenderTimeResults()
    {
        uint64_t buffer[2];

        VkResult result = vkGetQueryPoolResults(device, m_PerfQueryPool,
        ...
        {
            mTimeQueryResults[0] = (buffer[1] - buffer[0]);
            m_ExecutionTime = float(mTimeQueryResults[0])/ 1000000000.0;
        }
        else
        {
       ...
       // vkResetQueryPool(device, m_PerfQueryPool, 0, 2);
    }
    ...
    void mainLoop() 
    {
        ...
        while (!glfwWindowShouldClose(window)) 
        {
            ...
            if (currentTime - lastTime >= 1.0)
            {
                ...
                if (aprCount < 61)
                {
                    apr[aprCount].frameRate = static_cast<float>(nbFrames);
                    apr[aprCount].second = static_cast<float>(aprCount);
                    apr[aprCount].gms = m_ExecutionTime;
                    aprCount++;
                }
                std::cout << "FrameNumber:" << m_FrameNumber << " FRate:" << 1000.0 / double(nbFrames) << " ms/F, " << " FPS:" << nbFrames << " F/s." << std::endl;
                nbFrames = 0;
                lastTime += 1.0;
            }
            ...
            if (seconds > 60)
                break;
            apr[aprCount].frameTime = float(timerstep->elapsed());
        }

        vkDeviceWaitIdle(device);
        std::string filename = "../../../../FPIBGData/mmrr.csv";
        {
            float totFps = 0.0;
            float totTime = 0.0;
    ...
    void cleanup() {
        ...
        vkDestroyQueryPool(device, m_PerfQueryPool, VK_NULL_HANDLE);
    ...
    void pickPhysicalDevice() {
        ...
        for (const auto& device : devices) 
        {
            ...
            if (strcmp(Properties.deviceName, physDevName.c_str()) == 0)
            {
    ...
    void createRenderPass() {
        ...
        // The next two fields specify the operations to 
        // wait on and the stages in which these operations occur.
    ...
    QueueFamilyIndices findQueueFamilies(VkPhysicalDevice device) {
        ...
        for (const auto& queueFamily : queueFamilies) {
            ...
            {
                m_TimeStampDivisor = queueFamily.timestampValidBits;
                std::cout  << "VkQueryPool Perf Counters supported, count is:" << queueFamily.timestampValidBits
                    << std::endl;
            }
            else
                std::cout << "VkQueryPool Perf Counters NOT supported:" << queueFamily.timestampValidBits
                << std::endl;;
