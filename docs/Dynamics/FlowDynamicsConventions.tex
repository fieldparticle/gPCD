...
GLSL exporter now emits independent piston, general finite-line, and
piecewise parametric-curve wall support.  The horizontal and CD-nozzle
scenario wrappers compile and run through the Vulkan path.
...
deepest candidate for each physical wall flag is retained.  The CD nozzle now
uses piecewise cubic parametric segments while preserving the
same source-owned contact and piston rules.  Parametric markers use evaluator
ID 5.  Vulkan uses the same segment coefficients for wall response, and the
generator derives its visual OBJ from the same segment samples.
