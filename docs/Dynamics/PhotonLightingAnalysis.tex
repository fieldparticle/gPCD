...
\begin{itemize}
    ...
    \item material spectral fields through \texttt{spectral\_response},
          \texttt{spectral\_emission}, and \texttt{photon\_energy};
...
The active Vulkan implementation writes exactly one spatial cell per photon
event.  For the
...
from the cell id computed from the fragment world position.  The fragment
shader samples the current cell and six face-neighbor cells by distance to
cell centers.  A wider kernel may still be useful later for filtering or
smoothing, but it is not the first visible surface-lighting rule.
