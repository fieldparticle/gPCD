...
For each source particle \(i\), the implementation reads immutable frame-start
state, processes every detected contact \(j \in C_i\), calculates each contact
contribution, sums the contributions locally, and writes only source particle
\(i\):
\[
\Delta\mathbf p_i
=
...
Every source reads the same frame-start snapshot.  A source processes all of its
contacts before writing its velocity, and no source writes another particle.
The current implementation calculates source-local contact weights before
planning and vector-summing the contact impulses.
...
This source-local allocation is a known conservation failure when it changes
the physical pair impulse.  Two sources with different surrounding contact
sets calculate different weights for the same pair contact, so their
independently evaluated impulse magnitudes are not equal-and-opposite.
...
The current weighted raw impulse is:

\begin{equation}
J_{\mathrm{raw},i}
=
q_{\mathrm{pair}} A_i
\left(\frac{A_i}{\sum_j A_j}\right)
\Delta t.
\end{equation}

This is a description of the implemented rule, not a conservation claim.
Current \texttt{ThreeParticle} diagnostics show that this source-local weighted
raw impulse produces whole-system vector momentum and kinetic-energy drift.
