...
particle.  This is an explicit exception to the fully stateless geometric
target because the current overlap-area impulse needs a ledger to avoid
releasing more momentum than it removed during compression.

The exception is constrained:

\begin{itemize}[leftmargin=*]
    \item the value is owned by the source particle, not by a contact slot;
    \item it is not a shared pair reservoir;
    \item it is not a per-contact reservoir;
    \item it must not write the target particle;
    \item it may be used only to cap source-owned rebound/release impulse;
    \item it must remain representable in the GLSL-shaped particle structure.
\end{itemize}
...
This field stores source-owned internal normal momentum.  Contact slots may
report the current value or current-frame allocation, but slot index is not
contact identity and must not own persistent dynamics memory.
...
phase report, and diagnostics.  It must not be searched by target in order to
preserve dynamics memory.  Persistent source-owned memory belongs on the
particle.

When a source-owned total ledger is released across multiple contacts, release
capacity must be shared across the current contact set by overlap-area weight.
Slot order must not decide which contact receives the last remaining internal
momentum.
...
source-particle memory, not per-contact memory.
...
\begin{itemize}
    ...
    \texttt{ThreeParticleChase} momentum drift as the next active problem case.
