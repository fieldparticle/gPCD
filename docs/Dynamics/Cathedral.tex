...
The current photon-dust response is optical rather than mechanical.  When a
photon source crosses or overlaps a dust target, the dynamics performs a swept
intersection against the dust particle, reflects the photon direction at the
configured fixed speed, and places the photon at the reflected end-of-frame
position.  That event does not call the ordinary particle collision processor,
does not allocate a mechanical contact slot, does not add source-local force,
and does not accumulate internal contact momentum.  This prevents a photon from
remaining trapped inside a dust contact while the dust drifts away.
