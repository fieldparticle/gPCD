...
or another boundary target, the contacted boundary proxy stores the surface
material color in persistent boundary-space state.  The Python renderer then
colors existing boundary markers by sampling nearby valid proxies on the same
surface.  It does not create triangles or visible surface-cell geometry.
...
application uses the \texttt{FPML.*} shader family.  \texttt{FPML.comp}
contains the photon-aware compute schedule and deposits persistent surface
color into the lighting resource.  \texttt{FPML.vert} samples that persistent
state and spreads it over existing boundary particles.
...
per boundary particle.  The resource is bound only in the \texttt{ParticleLighting}
application and only consumed by the \texttt{FPML.*} shader family.  The
validated state is that \texttt{LightingBall} in Vulkan runs and visually
