...
\begin{description}
    ...
    boundary-particle walls by finding particles with positive
    evaluator-valued \texttt{ptype}, not by branching on this string.
...
\texttt{Data.w} is zero, and marker stiffness is zero.  Each marker writes its
evaluator ID directly to \texttt{pdata.ptype}: horizontal is 1, vertical is 2,
and CD nozzle is 3.  Python and the Vulkan loader copy that value into runtime
\texttt{ptype} and \texttt{Data.z}.  \texttt{pdata.temp\_vel} remains reserved.
These are current implementation facts, not configurable fields.
