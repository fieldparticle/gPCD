...
update model-owned state when required
\end{verbatim}
...
\texttt{<model>Dynamics.py} should avoid Python-only helper names unless the helper
is also expected to exist in GLSL.

Before adding or renaming any dynamics field, buffer, function, or constant, check
the GLSL source files first.  The GLSL names are the naming authority for each
portable dynamics path:
