...
class Base:
    ...
    def clamp_pair_penetration_limit(self, iterations=3):
        ...
        # Relax pair positions a few times because correcting one pair can
        # slightly disturb another in multi-particle contact.
        for _ in range(max(1, int(iterations))):
            ...
            for i in range(len(self.particles)):
                ...
                for j in range(i + 1, len(self.particles)):
                    ...
                    # Development limit: do not allow overlap deeper than the
                    # leading edge reaching the other particle's center.
                    target_distance = max(particle_i["radius"], particle_j["radius"])
                    if d >= target_distance:
                        continue

                    # If centers coincide, choose a stable correction direction.
                    if d <= 1.0e-12:
                        nx = 1.0
                        ny = 0.0
                    else:
                    ...
                    # Split the positional correction inversely by mass, so the
                    # lighter particle moves farther than the heavier one.
            ...
            # Stop once a full pass finds no pair beyond the penetration limit.
            if not adjusted:
                break
    ...
    def particle_contact(self, x1, y1, x2, y2, r1, r2):
        # Center-to-center offset from particle 1 to particle 2.
        dx = x2 - x1
        dy = y2 - y1
        ...
        # Contact starts only when the particle disks overlap.
        sum_r = r1 + r2
        if d >= sum_r:
            return None

        # Unit contact normal points from particle 1 toward particle 2.
        # If the centers are effectively coincident, pick a stable default.
        if d < 1.0e-14:
            nx = 1.0
            ny = 0.0
            alpha = 0.0
        ...
        # Development simplification: assume equal-radius behavior and do not
        # allow one particle to engulf the other. The deepest allowed contact is
        # where a leading edge reaches the other particle's center.
        max_delta = min(r1, r2)
        delta = min(sum_r - d, max_delta)
        proxPercent = 1.0 - delta / max(min(r1, r2), 1.0e-12)

        # Use the capped penetration depth for area geometry. The returned
        # center distance `d` below remains the actual center distance.
        contact_d = sum_r - delta

        # Partial overlap: compute the circular lens area as the sum of the
        # two circular sectors minus the shared triangular kite.
        term1 = r1 * r1 * math.acos(max(-1.0, min(1.0, (contact_d * contact_d + r1 * r1 - r2 * r2) / (2.0 * contact_d * r1))))
        term2 = r2 * r2 * math.acos(max(-1.0, min(1.0, (contact_d * contact_d + r2 * r2 - r1 * r1) / (2.0 * contact_d * r2))))
        term3 = 0.5 * math.sqrt(
            max(
                0.0,
                (-contact_d + r1 + r2) * (contact_d + r1 - r2) * (contact_d - r1 + r2) * (contact_d + r1 + r2),
            )
        )
        area = term1 + term2 - term3

        # Normalize by the smaller particle's disk area.
        tot_area = math.pi * min(r1, r2) ** 2
        area_pcnt = area / max(tot_area, 1.0e-12)

        # Return direction, angle, capped penetration, overlap area, normalized
        # overlap/proximity measures, and the actual center distance.
