...
class FieldBase(_Base):
    ...
    def do_substep(self, sub_dt):
        # Sample the current geometric interactions before applying any motion
        # or momentum transfer for this substep.
        ...
        # Keep contact counts available for tracing, UI display, and collision
        # start/end detection later in the substep.
        ...
        # During contact, recursively split the substep if the configured
        # refinement or speed-based requirement asks for finer sampling.
        refine_steps = max(
        ...
        # Track which remembered contacts remain active; inactive memories are
        # cleaned up after the current interaction pass.
        active_pair_keys = set()
        active_wall_keys = set()

        # No active geometry means free motion. Reset per-step internal
        # bookkeeping and, if a collision was ending, close the final report.
        if self.active_contact_count == 0:
            ...
            # If contacts were active on previous substeps, this no-contact
            # sample marks the end of the collision event.
            ...
            # With no contact forces to apply, particles move ballistically for
            # this substep, then are clamped back inside supported limits.
            for particle in self.particles:
            ...
            # Emit one trailing free row so traces show the separation point
            # after a contact-active interval.
            ...
            # Once separated, prior contact normals/areas should not influence
            # the next independent collision event.
