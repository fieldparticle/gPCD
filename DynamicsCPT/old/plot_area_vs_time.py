...
def plot_trace(trace_path: Path, show_window: bool) -> None:
    ...
    output_path = PLOTS_DIR / f"{trace_path.stem}_area_vs_time.png"

    fig, axes = plt.subplots(2, 1, figsize=(9, 7), sharex=True)

    axes[0].plot(time_values, overlap_area, color="#b45309", linewidth=2)
    axes[0].set_title(f"Overlap Area vs Time: {trace_path.stem}")
    axes[0].set_ylabel("Overlap Area")
    axes[0].grid(True, alpha=0.3)

    axes[1].plot(time_values, swept_area, color="#2563eb", linewidth=2)
    axes[1].set_title(f"Swept Area vs Time: {trace_path.stem}")
    axes[1].set_xlabel("Time")
    axes[1].set_ylabel("Swept Area")
