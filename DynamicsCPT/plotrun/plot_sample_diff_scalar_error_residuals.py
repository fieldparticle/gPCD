...
OUTPUT_FILE = PLOTS_DIR / "sample_diff_scalar_error_residuals.png"
...
def linear_fit_through_origin(x_values: list[float], y_values: list[float]) -> float:
...
def main() -> None:
    ...
    fit_slope = linear_fit_through_origin(sample_diffs, scalar_errors)
    predicted_scalar_errors = [fit_slope * diff for diff in sample_diffs]
    residuals = [actual - predicted for actual, predicted in zip(scalar_errors, predicted_scalar_errors)]
    ...
    axes[1].plot(study_indices, abs_scalar_errors, marker="o", linewidth=1.8, color="#dc2626")
    axes[1].set_yscale("log")
    axes[1].set_title("Ending Scalar Momentum Error Magnitude vs Study")
    axes[1].set_ylabel("|Scalar Momentum Error|")
    axes[1].grid(True, alpha=0.3, which="both")

    axes[2].plot(study_indices, residuals, marker="o", linewidth=1.8, color="#059669")
    axes[2].axhline(0.0, color="#64748b", linewidth=1.0, linestyle="--")
    axes[2].set_title(f"Residuals After Signed Error Fit To Sample Difference (slope={fit_slope:.2e})")
    ...
    for study_index, residual in zip(study_indices, residuals):
        axes[2].annotate(
            str(int(study_index)),
            (study_index, residual),
            textcoords="offset points",
            xytext=(0, 6),
            ha="center",
            fontsize=8,
