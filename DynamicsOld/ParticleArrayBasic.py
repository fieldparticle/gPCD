...
class ParticleArrayBasic():
    ...
    def calculate_collision_acceleration(self,pnum):
        ...
        for cc in self.pary[pnum].collision_list:
            if cc.col_flag == True:
                ...
                pf = cc.pen_factor
                area_diff = self.pary[pnum].prev_area-self.pary[pnum].cur_area
                self.pary[pnum].area_diff+=area_diff
               
                #print(f"pf:{pf}")
                # Mulitply penetration factor by the particle velocity prior to the collision
                v_coll = (((pf)*cc.vel_mag_in))
                v_coll = -area_diff*self.pary[pnum].mom_per_u_area*100
                if abs(v_coll) < 1E-2:
                    v_coll = -v_coll*100.0
                if(pnum==0):
                    #print(self.pary[pnum].area_diff) 
                    print(v_coll) 
             ...
             #print(col_dir)
                ...
                self.pary[pnum].internal_mom = np.linalg.norm([self.pary[pnum].tot_collision_x_acc,
                                                                 self.pary[pnum].tot_collision_y_acc])
                
                self.pary[pnum].internal_momx = self.pary[pnum].tot_collision_x_acc
                self.pary[pnum].internal_momy = self.pary[pnum].tot_collision_y_acc
    ...
    def move(self):
        for ii in self.pary:
            ...
            if ii.col_flag == False and ii.internal_mom != 0.0:
                #ii.vx += ii.internal_momx 
                #ii.vy += ii.internal_momy 
                self.remained_momentum = ii.internal_mom
                #ii.vx -= ii.area_diff*ii.mom_per_u_area
                #ii.internal_momx = 0.0
                #ii.internal_momy = 0.0
                ii.internal_mom = 0.0
        ...
        for src in self.pary:
            ...
            try:
                with open(file_name, 'w', newline='') as csvfile:
                    csvfile.write("P,t,vx,vy,pen_factor,v_mag,vel_mag_in,imom,v_ang,imomx,imomy\r\n")
                    for ii in range(len(src.stor_t)):
                        ...
                        line += f"{src.stor_internal_momx[ii]:0.4f},{src.stor_internal_momy[ii]:0.4f}\r\n"
                        csvfile.write(line)
            except BaseException as e:
                print(f"save_data error {e}")
            print(f"Saved particle {src.pnum}")    
    ...
    def set_storage_variables(self,src):
        ...
        src.stor_internal_momx.append(src.internal_momx)
        src.stor_internal_momy.append(src.internal_momy)
