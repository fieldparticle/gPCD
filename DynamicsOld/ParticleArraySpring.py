...
class ParticleArraySpring():
                ...
                if src != trg:
                    ...
                    if (self.check_collision(src,trg) == True):
                        ...
                        if(self.pary[src].col_flag == False):    
                            ...
                            self.count = 0
                            self.pary[src].m_vx = 0.0
                            self.pary[src].collision_count = 0
                            self.pary[src].vx_in = self.pary[src].vx
                            
                            self.pary[src].vel_mag_in = np.linalg.norm([self.pary[src].vx,self.pary[src].vy])
                        ...
                        self.calculate_collision_acceleration(src)
                         ...
                         if(self.pary[src].col_flag == True):
                             # Reset flag after collision
                             self.pary[src].col_flag = False


   
    def to_index(self,val):
        #print(f"ciel:{math.ceil(1000*val)},floor:{math.floor(1000*val)},int:{math.floor(1000*val)}")
        return math.ceil(1000*val)
    ##################################################################
    #
    ...
    def calculate_collision_acceleration(self,pnum):
        ...
        for cc in self.pary[pnum].collision_list:
            ...
            v_coll = pf
            try:
                index = self.to_index(pf*self.pary[pnum].time_in_collison_factor)
                self.pary[pnum].collision_count += 1
                v_coll = self.pary[pnum].repl[index]
                self.pary[pnum].stor_attr.append(v_coll)
            except BaseException as e:
                print(f"calculate_collision_acceleration{e}")
                
            #print(self.pary[pnum].collision_count)
    ...
    def move(self):
        for ii in self.pary:
            ...
            if len(ii.collision_list) > 0:
                ...
                # do collsion acceleration                
                ii.vx = ii.vx + ii.tot_collision_x_acc
                ii.vy = ii.vy + ii.tot_collision_y_acc

                # Bleed off for bonding
                ii.vx = ii.vx - get_sign(ii.vx)*ii.attr
                ii.m_vx = abs(ii.vx)-abs(ii.vx_in)
      
                #print(f"sto_mom:{ii.m_vx:.4f}, f:act_mom:{abs(ii.vx)-abs(ii.vx_in):.4f}")
            # Get the new angle
            ii.vel_ang = atan360(ii.vx,ii.vy,0.0)
            ii.vel_mag = np.linalg.norm([ii.vx,ii.vy])

            # If there is stored momentum
            if(ii.col_flag == False):
                if ii.pred_mom_xout != 0.0:
                    sgn = get_sign(ii.vx)
                    #print(abs(ii.vx_in) - abs(ii.vx))
                    if abs(ii.pred_mom_xout) < abs(ii.vx):
                        ii.vx = ii.pred_mom_xout
                        ii.pred_mom_xout = 0.0
                    else:
                        ii.vx = ii.pred_mom_xout
                        ii.pred_mom_xout = 0.0
 ...
 ##################################################################
    ...
    def start(self):
        ...
        df = self.read_bond_profile()
        if df.empty == False:
            repl = df["r"].values
        else:
            repl = None

        if "in_collison_factor" in self.itemcfg:
            icf = self.itemcfg.in_collison_factor
        else:
            icf = 0.1
    ...
    def read_bond_profile(self):
        if "bond_profile" in self.itemcfg:
            df = pd.read_csv(self.itemcfg.bond_profile)
            return df
        else:
            df = pd.DataFrame
            return df
    ...
    def save_data(self,data_file_name):
        for src in self.pary:
            ...
            try:
                with open(file_name, 'w', newline='') as csvfile:
                    csvfile.write("P,t,vx,vy,col_count,pen_factor,attr,v_mag,cvmag,v_ang,vrel,v_collx,vcolly\r\n")
                    for ii in range(len(src.stor_t)):
                        line = f"{src.stor_pnum[ii]},{src.stor_t[ii]:0.4f},{src.stor_vx[ii]:0.4f},{src.stor_vy[ii]:0.4f},"
                        line += f"{src.stor_col_count[ii]},{src.stor_pen_factor[ii]:0.4f},{src.stor_attr[ii]:0.4f},{src.stor_v_mag[ii]:0.4f},"
        ...
        for src in self.pary:
            file_name = data_file_name+f"ar{src.pnum}.csv"
            try:
                with open(file_name, 'w', newline='') as csvfile:
                    csvfile.write("P,a,r,sum\r\n")
                    for ii in range(len(src.attr)):
                        line = f"{src.pnum},{src.attr[ii]:0.4f},{src.repl[ii]:0.4f},"
                        line += f"{src.arsum[ii]:0.4f}\r\n"
                        csvfile.write(line)
                        #print(ii)
            except BaseException as e:
                print(f"save_data():{e}")
 ...
 ##################################################################
    ...
    def set_storage_variables(self,src):
        ...
        if len(src.collision_list) > 0:
            ...
            src.stor_col_count.append(src.collision_count)
        else:
            ...
            src.stor_col_count.append(0)
            src.stor_attr.append(0)
